...
class ChangeLogController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Check permission
        $this->authorize('add', app($dataType->model_name));
        $chk = Changelog::whereDate('created_at', now())->latest()->first();
        $latest = Changelog::latest()->first();
        ...
        if ($count==0) {
            ...
            $v2 = explode('.',$latest->version)[1];
            if($chk){
                $v= $chk->version;
            }else{
                if($request->large_change=='on'){
                    $v2 = $v2+1;
                    $v3=0;
                }else{
                    $v3 = explode('.',$latest->version)[2]+1;
                }
                $v = $v1 . '.' . $v2 . '.' . $v3;
            }
        }
        ...
        return redirect(route('voyager.changelogs.index'))->with([
