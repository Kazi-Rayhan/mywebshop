                                            ...
                                            <td class="no-sort no-click bread-actions">
                                                    ...
                                                    @if (!method_exists($action, 'massAction'))
                                                        @if ($data)
                                                            ...
                                                            @can($action->getPolicy(), $data)
                                                                @if ($action->getTitle() == 'Edit' && $data->status == 2)
                                                                    @if ($data->is_company && !$data->refund > 0)
                                                                        <a href="{{ route('admin.orders.refund', $data) }}"
                                                                            title="Refund" {!! $action->convertAttributesToHtml() !!}>
                                                                            <i></i> <span
                                                                                class="hidden-xs hidden-sm">Refund</span>
                                                                        </a>
                                                                    @endif
                                                                @else
                                                ...
                                                @if ($data->is_company)
                                                    <div class="btn-group d-block mt-2">
                                                        @if (!in_array($data->two_payment_status, ['2', '0']))
                                                            <a class="btn btn-primary btn-sm"
                                                                href="{{ route('admin.orders.fulfiled', $data) }}"><i
                                                                    class="fa fa-check-circle mr-2"></i>{{ __('words.fulfill') }}</a>

                                                            @if ($data->status != 2)
                                                                <a class="btn btn-primary btn-sm"
                                                                    href="{{ route('admin.orders.cancel', $data) }}"><i
                                                                        class="fa fa-times-circle mr-2"></i>{{ __('words.cancel') }}</a>
                                                            @endif
                                                        @else
                                                            @if ($data->status == 2)
                                                                <a class="btn btn-primary btn-sm"
                                                                    href="{{ (new App\Payments\TwoPayment($data))->invoice() }}"><i
                                                                        class="fa fa-file-invoice mr-2"></i>{{ __('words.invoice') }}</a>
                                                            @endif

                                                            <small class="badge badge-info">
                                                                @if ($data->two_payment_status == '0')
                                                                    {{ __('words.canceld') }}
                                                                @elseif($data->two_payment_status == '1')
                                                                    {{ __('words.confirmed') }}
                                                                @else
                                                                    {{ __('words.fulfiled') }}
                                                                @endif
