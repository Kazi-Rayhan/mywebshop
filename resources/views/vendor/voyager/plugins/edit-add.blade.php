                    ...
                    <form role="form" class="form-edit-add"
                        ...
                        <div class="panel-body">
                            ...
                            @foreach ($dataTypeRows as $row)
                                ...
                                @if ($dataTypeContent->paymentMethod == 'elavon')
                                    @if (in_array($row->field, ['quickpay_api_key', 'quickpay_secret_key']))
                                        @php
                                            continue;
                                        @endphp
                                    @endif
                                ...
                                <div class="form-group @if ($row->type == 'hidden') hidden @endif col-md-{{ $display_options->width ?? 12 }} {{ $errors->has($row->field) ? 'has-error' : '' }}"
                                    ...
                                    @else
                                        ...
                                        @else
                                            <div class="col-md-4">
                                                <x-form.input type="text" name="company_address[city]" label="City"
                                                    :value="@$dataTypeContent->company_address->city" />
                                            </div>

                                            <div class="col-md-4">
                                                <x-form.input type="text" name="company_address[street]" label="Street"
                                                    :value="@$dataTypeContent->company_address->street" />
                                            </div>

                                            <div class="col-md-4">
                                                <x-form.input type="text" name="company_address[zip]" label="Zip"
                                                    :value="@$dataTypeContent->company_address->zip" />
                                            </div>
                                        @endif
                                    @endif
                                    ...
                                    @foreach (['elavon_merchant_alias', 'elavon_public_key', 'elavon_secret_key'] as $field)
                                        <div class="col-md-6">
                                            <x-form.input type="text" name="meta[{{ $field }}]"
                                                ...
                                                value="{{ old($field, $dataTypeContent->$field) }}" />
                                        </div>
                                    @endforeach
                                </div>
